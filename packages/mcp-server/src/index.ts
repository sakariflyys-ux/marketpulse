/**
 * Synergilon MCP server (stdio).
 *
 * Exposes the shared tool definitions from @synergilon/db/tools over the
...
async function main(): Promise<void> {
  ...
  const server = new McpServer({ name: "synergilon", version: "0.1.0" });

  // Widened to the generic definition type so one loop registers every tool.
  for (const def of synergilonTools as readonly ToolDefinition[]) {
    server.registerTool(
      def.name,
  ...
  log(`ready (${synergilonTools.map((t) => t.name).join(", ")})`);
