...
- **What it does not provide:** revenue or traffic. `monthlyRevenue` / `monthlyTraffic` are **null** for live stores and nothing is put in their place. (Phase 7 shipped a "revenue estimate" derived from catalogue size and prices; it was off by orders of magnitude for large brands and was removed in Phase 7.1. The `revenueEstimate` / `estimateConfidence` columns remain in the schema but are no longer written or shown.) Live cards show what is measured instead: product count and price range. Trending for live stores ranks on the observable product-count change between snapshots.
- **Store name:** resolved from `og:site_name`, then an `Organization`/`WebSite` name in `application/ld+json`, then the `Shopify.shop` handle, then a short `<title>` lead, then the domain — never the raw SEO title, which is kept on `pageTitle`.
- **Catalogue crawl:** `/products.json` is paged 250 at a time (`page=N`, falling back to `since_id` when a store ignores `page`) up to 40 pages. Past that, `productCountTruncated` is set and the UI shows `10,000+`. Prices are aggregated over every page.
- **Categories:** raw `product_type` values and tags are stored on `rawTags` (most common first) and normalised into one of Apparel, Footwear, Beauty, Home, Food & Drink, Fitness, Accessories, Jewelry, Pets, Electronics or Other via keyword rules in `packages/db/src/sources/shopify/categories.ts`, weighted by product count.
- **Bad neighbours:** a 429 is retried once (after `Retry-After`, else 30 s); a 403 marks the tracked entity blocked for 30 days so nightly runs skip it; a non-Shopify domain or two consecutive no-response runs deactivate the entity with a reason, visible on `/ingest`.
